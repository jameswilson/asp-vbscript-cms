...
class SiteModules
	...
	private function initModulesDictionary()
		...
		if rs.state > 0 then 
			if rs.EOF and rs.BOF then
				...
				do while not rs.EOF
					if not sd.exists(""&rs("ModID")) then 
	...
	public function getModAddress(itemId)
		...
		if len(itemId) > 0 then 
			if myModulesDict.exists(""&itemId) then 
				result = myModulesDict.item(""&itemId)
			else
			...
			do until rs.EOF
				...
				for i=0 to counter-1
					...
					if not sd.exists(key) then
						sd.add key, val
						if key = "CustomSettings" then CreateDictionary sd,val,CUSTOMSETTINGS_RECORD_DELIMITER,CUSTOMSETTINGS_FIELD_DELIMITER,adDictOverwrite
					else
		...
		if rsPageModules.state > 0 then
			if rsPageModules.EOF or rsPageModules.BOF then
				...
				do until rsPageModules.EOF
					trace("[ Module:"&rsPageModules("ModName")&", Location:'"&rsPageModules("Location")&"', CustomSettings:'"&server.HTMLEncode(""&rsPageModules("CustomSettings"))&"']")
	...
	public sub executeMod(strHandler, strCustomSettings)
		...
		if isNull(strCustomSettings) or strCustomSettings = "" then 
			...
			session("ModuleCustomSettings") = ""&strCustomSettings
		end if
