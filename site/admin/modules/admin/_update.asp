...
'					"<li><a href='?create'>Upload a New "&Pcase(strContent)&" </a></li>"& vbcrlf & _
'					"<li>View changes on:<ul>" & vbcrlf 
'				if instr(myForm.getValue("PageIDs"),":0:")> 0 then strSuccess = strSuccess &"<li><a href='"&objLinks.item("SITEURL")&"'>Home Page</a></li>"
'				set rs = db.getRecordSet("SELECT * FROM tblPages ORDER BY Active, MainMenu, MenuIndex, ParentPage;")
'				if not (rs.EOF and rs.BOF) then
'					rs.movefirst
'					do until rs.EOF
'						if instr(myForm.getValue("PageIDs"),":"&rs("PageId")&":")> 0 then strSuccess = strSuccess &"<li><a href='"&objLinks.item("SITEURL")&"/"&rs("PageFileName")&"'>"&rs("PageName")&"</a></li>"
'						rs.movenext
'					loop
'				end if
