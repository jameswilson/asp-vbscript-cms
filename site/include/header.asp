<%
	...
	dim title : title = objLinks.item("TITLE")
	dim slogan : slogan = objLinks.item("SLOGAN")
	dim logoTitle : logoTitle = title&": "&slogan
	logo.Path = objLinks.item("LOGO")
	if logo.fileExists then 
		writeln(indent(2)& a(objLinks.item("SITEURL"), mediaFile(logo.Url,logoTitle,"logo","","") ,logoTitle, "logo"))
	end if
	...
	if user.isAdministrator() then
		...
		if page.isAdminPage() then
			writeln(indent(4)& CreateNavLink(objLinks.item("ADMIN_DIR")&"/users/users.asp?edit="&user.getId(), user.GetFullName(),"Edit your account.",null)&"</li>")		
			...
			writeln(indent(4)& CreateNavLink(objLinks.item("ADMIN_DIR"),"Site Admin","Log into "&objLinks.item("PRODUCT_BRANDING")&": Administration Panel","last")&"</li>")
		end if
